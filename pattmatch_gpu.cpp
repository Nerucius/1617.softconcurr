...
CImg<float> pattern_matching(CImg<unsigned char> &img, CImg<unsigned char> &pat) {
	...
	if (status != CL_SUCCESS) {
		...
		clGetProgramBuildInfo(program, devices[0], CL_PROGRAM_BUILD_STATUS, sizeof(cl_build_status), &build_status,
							  NULL);
...
//	cl_uint work_dimension = 1;
	...
	cout << "Execution of kernel time at GPU = " << total_time / 1000000.0 << "ms" << endl;
	...
	cout << "Elapsed time of pattmatch function = " << mtime << "ms" << endl;
